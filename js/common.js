...
( function( $ ) {
	...
	*/
	...
	/*
		Header Sticky
	*/
	if($('.fid-header').length) {
		$(window).on('scroll', function(){
			if ( $(window).scrollTop() > 48 ) {
				$('.fid-header').addClass('fixed');
			} else {
				$('.fid-header').removeClass('fixed');
			}
		});
	}
	if(!$('.fid-started-inner .kf-parallax-bg').length && !$('.fid-started-slider').length) {
		$('.fid-navbar').addClass('inner-navbar');
	}

	/*
		Header Menu Button
	*/
	$('.fid-header').on('click', '.fid-menu-btn', function(){
		if($(this).hasClass('active')) {
			$(this).removeClass('active');
			$('.fid-header').removeClass('show');
			$('.fid-header').addClass('no-touch');
			setTimeout(function(){
				$('.fid-header').removeClass('animated');
			}, 500);
			setTimeout(function(){
				$('.fid-header').removeClass('opened');
				$('.fid-header').removeClass('no-touch');
			}, 1000);
		}
		else {
			$(this).addClass('active');
			$('.fid-header').addClass('animated');
			$('.fid-header').addClass('no-touch');
			setTimeout(function(){
				$('.fid-header').addClass('opened');
			}, 500);
			setTimeout(function(){
				$('.fid-header').addClass('show');
				$('.fid-header').removeClass('no-touch');
			}, 1000);
		}
		return false;
	...
	*/
	$('.fid-navbar-mobile .fid-main-menu').on('click', '.has-children > i', function(){
	...
	*/
	...
	var mainSliderSelector = new Swiper('.fid-started-slider .swiper-container', {
		slidesPerView: 1,
		spaceBetween: 0,
		loop: true,
		speed: 1000,
